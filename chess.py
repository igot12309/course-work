	...
	if tkstart != () and tkend != ():
		...
		if not check_mate:
			...
			if error == 0:
				...
				endbutton_color = ''
				error = -1
				display_player_color = "Белые ходят" if current_color == 1 else "Черные ходят"
...
labelZahl1 = Label(tk, text="Белые ходят", bg = 'white', width = 30)
labelZahl1.config(font=(10))
labelZahl1.grid(row=1, column=9, sticky='w')
labelZahl2 = Label(tk, text='История ходов', bg = 'white', width = 30)
...
restartgame_button = Button(tk, text='Перезапуск', bg='white', height=1, width=10, command=lambda: [restart_game()])
